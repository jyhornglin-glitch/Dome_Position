...
def create_manual():
    ...
    intro_text = (
        ...
        "演繹人員只需輸入自己所在的「起點座標」，系統即會自動繪製並計算在 8 大不同演繹定位點 "
        "（基本、圓形、行願、靜思、05-1有法船（點一盞燈）、05-2無法船（菜市場5毛錢）、05-3無法船/有法船(是諸眾生)、四弘誓願）中的相對位置、行進方向、步數以及歌詞段落。 "
    ...
    p2 = (
        ...
        "2. 指引線路徑：黃色粗線與箭頭為目前步驟的行進動線。勾選「顯示完整 8 點軌跡」後，會同時以各步驟的專屬色彩顯示完整的連續移動路線。"
    )
    pdf.set_left_margin(20)
    ...
    p3 = (
        ...
        "2. 隊形步驟與頁籤切換按鈕：\n"
        "   - 「上一個」與「下一個」按鈕：位於地圖上方，用於在 8 個演繹定位點步驟間切換。\n"
    ...
    p4 = (
        ...
        "1. 跑位引導步驟：\n"
        "   - 在「跑位引導」頁籤中，系統以步驟清單詳細列出 8 個演繹步驟的行進指引。\n"
        "   - 每一步驟包含：目標絕對座標、行進方向提示（如：向右後 3.5 步）、直線距離步數，以及對應的歌詞OS段落。\n\n"
        "2. 隊形詳情圖卡：\n"
        "   - 在「隊形詳情」頁籤中，列出了 8 大隊形定位點的專屬圖卡。\n"
    ...
    p5 = (
        "1. 查看與下載所有圖：\n"
        "   - 點擊「查看與下載所有圖」按鈕，會開啟彈出視窗預覽 8 個演繹定位點的縮圖。\n"
        "2. 高解析度 PDF 下載：\n"
